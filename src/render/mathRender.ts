...
import { Constants, Lute } from "siyuan";
import { hasNextSibling, hasPreviousSibling } from "./utils";
import { hasClosestBlock } from "./utils";
    ...
    addScript(`${cdn}/js/katex/katex.min.js?v=0.16.9`, "protyleKatexScript").then(() => {
        addScript(`${cdn}/js/katex/mhchem.min.js?v=0.16.9`, "protyleKatexMhchemScript").then(() => {
            ...
            mathElements.forEach((mathElement: HTMLElement) => {
                ...
                try {
                    renderElement.innerHTML = window.katex.renderToString(window.Lute.UnEscapeHTMLStr(mathElement.getAttribute("data-content")), {
